...
- **Placing a new node? Call `@shared/placement`, never a local `{x, y}` rule.** Every path that
  ...
  that frame, which grows to hold it — unless it waits on `--after` deps, in which case it stays
  top-level beside them. The live, cold and headless paths must agree on where it lands, and
  `test/acceptance/placement-parity.test.ts` fails when they do not. Moving EXISTING
